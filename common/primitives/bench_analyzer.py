...
class bench_analyzer(object):
    def __init__(self, score_info, create_score_dict, file_name=None, json_data=None):
        '''
        file_name:         string:   name of file containing json data for bench output
        score_info:        dict:     key is test name, val is dict of the form {"normalizers": val, "weight": val}
            normalizers: normalizers to turn score into percentage of "top performance 
                                     capable" ex. 10000.0
            weights:     weights for each score to get weighted avg ex. 2.0
        create_score_dict: function: meant to take raw json data from bench and turn it 
                                     into a dict containing only names of tests to be included and their scores
        '''
    ...
    def analyze(self):
        normal_scores_dict = {key: (100*float(val)/float(self.score_info[key]["normalizer"])) for key,val in self.score_dict.iteritems()}
        weighted_normal_scores_list = [self.score_info[key]["weight"]*val for key,val in normal_scores_dict.iteritems()]
	weight_total = sum([val["weight"] for val in  self.score_info.values()])
        self.json_data["overall_score"] = self._avg(weighted_normal_scores_list, weight_total)
